...
bool ObjModel::MaterialLib::Save(const String& prefix) const
{
	...
	for (int_t i = 0; i < (int_t)materials.size(); ++i) {
		...
		out << "newmtl " << mat.name << "\n"
			...
			<< "illum 1" << "\n"
			<< "Ns 1.000000" << "\n"
			<< "map_Kd " << Util::getFileFullName(mat.diffuse_name) << "\n";
		// save material maps
        DEBUG_EXTRA("save image %s", mat.diffuse_name.c_str());
		const bool bRet(mat.diffuse_map.Save(mat.diffuse_name));
        DEBUG_EXTRA("save image ok");
		if (!bRet)
			return false;
	}
	return true;
