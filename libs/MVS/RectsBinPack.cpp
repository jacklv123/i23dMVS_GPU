...
bool MaxRectsBinPack::InsertPartial(RectArr& rects, RectArr& newRects, IndexArr& indices, Group& newGroup, FreeRectChoiceHeuristic method)
{
	while (!rects.IsEmpty()) {
        int bestScore1 = std::numeric_limits<int>::max();
		int bestScore2 = std::numeric_limits<int>::max();
		...
		// if no place found...
		if (bestRectIndex == NO_IDX) {
		    DEBUG_EXTRA("remain %u rects", rects.GetSize());
			return false;
		}
        ...
        newGroup.Insert(indices[bestRectIndex]);
		newRects[indices[bestRectIndex]] = bestNode;
		rects.RemoveAt(bestRectIndex);
		indices.RemoveAt(bestRectIndex);
	}
