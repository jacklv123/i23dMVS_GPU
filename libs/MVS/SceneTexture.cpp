...
public:
	...
	void GenerateTexture(bool bGlobalSeamLeveling, bool bLocalSeamLeveling, unsigned nTextureSizeMultiple, unsigned nRectPackingHeuristic, Pixel8U colEmpty, bool bMultiTexture, unsigned nMaxTextureSize);
...
MeshTexture::MeshTexture(Scene& _scene, unsigned _nResolutionLevel, unsigned _nMinResolution)
	...
	textures(_scene.mesh.textures),
	groups(_scene.mesh.groups),
...
void MeshTexture::GlobalSeamLeveling()
{
    ...
    DEBUG_EXTRA("adjust texture patches using the correction color: %u", numPatches);
	// adjust texture patches using the correction colors
	#ifndef TEXOPT_USE_OPENMP
	#pragma omp parallel for schedule(dynamic)
	for (int i=0; i<(int)numPatches; ++i) {
	#else
	for (unsigned i=0; i<numPatches; ++i) {
	#endif
	//	DEBUG_EXTRA("adjust texture patches %d", i);
	...
	//		DEBUG_EXTRA("%d", idxFace);
	//		DEBUG_EXTRA("----%d", vertpatch2rows[face[0]].at(idxPatch));
	//		DEBUG_EXTRA("----%d", vertpatch2rows[face[1]].at(idxPatch));
	//		DEBUG_EXTRA("----%d", vertpatch2rows[face[2]].at(idxPatch));
			for (int v=0; v<3; ++v)
				data.colors[v] = colorAdjustments.row(vertpatch2rows[face[v]][idxPatch]);
...
void MeshTexture::GenerateTexture(bool bGlobalSeamLeveling, bool bLocalSeamLeveling, unsigned nTextureSizeMultiple, unsigned nRectPackingHeuristic, Pixel8U colEmpty, bool bMultiTexture, unsigned nMaxTextureSize)
{
    ...
    DEBUG_EXTRA("create texture with %u texturePatch", texturePatches.GetSize());
	// create texture
	{
		...
		unsigned textureSize(RectsBinPack::ComputeTextureSize(rects, nTextureSizeMultiple));
#ifdef _MULTI_TEX
        Mesh::GroupArr patchGroups;
        textureSize = std::min(textureSize, nMaxTextureSize);
	    cList<IDX, IDX, 0> indices(rects.GetSize());
	    std::iota(indices.Begin(), indices.End(), 0);
		RectsBinPack::RectArr newRects(rects.GetSize());
        while (true) {
            TD_TIMER_STARTD();
			bool bPacked(false);
            Mesh::Group newGroup;
		    MaxRectsBinPack pack(textureSize, textureSize);
			bPacked = pack.InsertPartial(rects, newRects, indices, newGroup);
		    DEBUG_EXTRA("create group: %u with size %u", patchGroups.GetSize(), newGroup.GetSize());
            patchGroups.Insert(newGroup);
            if(bPacked){
		        DEBUG_EXTRA("packed");
                rects.Swap(newRects);
                break;
            } else {
		        DEBUG_EXTRA("not packed");
            }
        }
        ...
        DEBUG_EXTRA("create texture image");
		// create texture image
		const float invNorm(1.f/(float)(textureSize-1));
#ifdef _MULTI_TEX
        groups.Resize(patchGroups.GetSize());
        textures.Resize(patchGroups.GetSize());
        FOREACH(i, textures) {
            textures[i].create(textureSize, textureSize);
		    textures[i].setTo(cv::Scalar(colEmpty.b, colEmpty.g, colEmpty.r));
        }
#else
		...
		#ifdef TEXOPT_USE_OPENMP
		#pragma omp parallel for schedule(dynamic)
		for (int_t j=0; j<(int_t)patchGroups.GetSize(); ++j) {
		#else
		FOREACH(j, patchGroups) {
		#endif
        const uint32_t idxGroup((uint32_t)j);
        const Mesh::Group& group = patchGroups[idxGroup];
        FOREACH(i, group) {
            const uint32_t idxRect(group[(uint32_t)i]);
#else
		#ifdef TEXOPT_USE_OPENMP
		#pragma omp parallel for schedule(dynamic)
		...
		FOREACH(i, rects) {
		#endif
			const uint32_t idxRect((uint32_t)i);
#endif
			const RectsBinPack::Rect& rect = rects[idxRect];
			const TexturePatch& texturePatch = texturePatches[idxRect];
			// copy patch image
...
#ifdef _MULTI_TEX
                patch.copyTo(textures[idxGroup](rect));
#else
...
#ifdef _MULTI_TEX
                groups[idxGroup].Insert(idxFace);
#endif
			}
		}
#ifdef _MULTI_TEX
		    DEBUG_EXTRA("create group: %u with size %u", idxGroup, groups[idxGroup].GetSize());
        }
...
// texture mesh
bool Scene::TextureMesh(unsigned nResolutionLevel, unsigned nMinResolution, float fOutlierThreshold, float fRatioDataSmoothness, bool bGlobalSeamLeveling, bool bLocalSeamLeveling, unsigned nTextureSizeMultiple, unsigned nRectPackingHeuristic, Pixel8U colEmpty, bool bMultiTexture, unsigned nMaxTextureSize)
{
	...
	{
		TD_TIMER_STARTD();
		texture.GenerateTexture(bGlobalSeamLeveling, bLocalSeamLeveling, nTextureSizeMultiple, nRectPackingHeuristic, colEmpty, bMultiTexture, nMaxTextureSize);
		DEBUG_EXTRA("Generating texture atlas and image completed: %u patches, %u image size (%s)", texture.texturePatches.GetSize(), mesh.textures[0].width(), TD_TIMER_GET_FMT().c_str());
	}
