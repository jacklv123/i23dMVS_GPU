...
bool Mesh::SaveOBJ(const String& fileName) const
{
    ...
    // multi texture
    #ifdef _MULTI_TEX
    FOREACH(idxGroup, groups) {
    DEBUG_EXTRA("store group %d", idxGroup);
    const Group& group = groups[idxGroup];
    ObjModel::Group& g = model.AddGroup(String::FormatString("material_%04u", idxGroup));
    g.faces.reserve(group.GetSize());
    FOREACH(j, group) {
        const uint32_t idxFace(group[(uint32_t)j]);
    #else
    ObjModel::Group& g = model.AddGroup(_T("material_0"));
	g.faces.reserve(faces.GetSize());
	FOREACH(idxFace, faces) {
    #endif
		const Face& face = faces[idxFace];
		ObjModel::Face f;
	...
	// store texture
	ObjModel::MaterialLib::Material* pMaterial(model.GetMaterial(g.material_name));
	ASSERT(pMaterial != NULL);
    #ifdef _MULTI_TEX
    //pMaterial->diffuse_map = textures[0];
    pMaterial->diffuse_map = textures[idxGroup];
    }
    #else
	pMaterial->diffuse_map = textureDiffuse;
    #endif

    DEBUG_EXTRA("save obj");
