...
namespace I23D { extern int g_nVerbosityLevel; }
...
#endif
#define INIT_WORKING_FOLDER	{I23D::Util::ensureValidDirectoryPath(WORKING_FOLDER); WORKING_FOLDER_FULL = I23D::Util::getFullPath(WORKING_FOLDER);} // initialize working folders
#define MAKE_PATH(str)		(WORKING_FOLDER+(str)) // add working directory to the given file name
#define MAKE_PATH_SAFE(str)	(I23D::Util::isFullPath((str).c_str()) ? String(str) : String(WORKING_FOLDER+(str))) // add working directory to the given file name only if not full path already
#define MAKE_PATH_FULL(p,s) (I23D::Util::isFullPath((s).c_str()) ? String(s) : String((p)+(s))) // add the given path to the given file name
#define MAKE_PATH_REL(p,s)	((s).compare(0,(p).size(),p) ? String(s) : String(I23D::String((s).substr((p).size())))) // remove the given path from the given file name
#define GET_PATH_FULL(str)	(I23D::Util::isFullPath((str).c_str()) ? I23D::Util::getFilePath(str) : String(WORKING_FOLDER_FULL+I23D::Util::getFilePath(str))) // retrieve the full path to the given file
...
#define DECOPT_SPACE(SPACE) namespace SPACE { \
	void init(); \
	void update(); \
	extern I23D::VoidArr arrFncOpt; \
	extern I23D::CConfigTable oConfig; \
}
#define DEFOPT_SPACE(SPACE, name) namespace SPACE { \
	I23D::CConfigTable oConfig(name); \
	typedef LPCTSTR (*FNCINDEX)(); \
	typedef void (*FNCINIT)(I23D::IDX); \
...
#define DEFVAR_OPTION(SPACE, flags, type, name, title, desc, ...) namespace SPACE { \
	...
	void update_##name() { \
		I23D::String::FromString(oConfig[title].val, name); \
	} \
	void init_##name(I23D::IDX idx) { \
		...
		I23D::String::FromString(opt.defval, name); \
	} \
	LPCTSTR index_##name() { \
...
typedef Vec3 X3D;
typedef I23D::cList<X3D, const X3D&, 0, 8192> X3DArr;

typedef I23D::cList<uint32_t, uint32_t, 0> IndexArr;
...
DEFINE_CVDATATYPE(I23D::Vec2f)
DEFINE_CVDATATYPE(I23D::Vec3f)
DEFINE_CVDATATYPE(I23D::Vec4f)
DEFINE_CVDATATYPE(I23D::Matrix2x2f)
DEFINE_CVDATATYPE(I23D::Matrix3x3f)
DEFINE_CVDATATYPE(I23D::Matrix3x4f)
DEFINE_CVDATATYPE(I23D::Matrix4x4f)

DEFINE_CVDATATYPE(I23D::Vec2d)
DEFINE_CVDATATYPE(I23D::Vec3d)
DEFINE_CVDATATYPE(I23D::Vec4d)
DEFINE_CVDATATYPE(I23D::Matrix2x2d)
DEFINE_CVDATATYPE(I23D::Matrix3x3d)
DEFINE_CVDATATYPE(I23D::Matrix3x4d)
DEFINE_CVDATATYPE(I23D::Matrix4x4d)
/*----------------------------------------------------------------*/
