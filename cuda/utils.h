...
#define synchronCheck {cudaDeviceSynchronize();cudaError_t err =cudaGetLastError();if (err != cudaSuccess) { printf("Cuda error: %d : %s at %s:%d\n", err,cudaGetErrorString(err) , __FILE__, __LINE__); assert(0);} }
